...
| **Remote doesn't fill the screen (margin on right/bottom, or bottom cut off)** | Wrong remote resolution vs the physical monitor under fractional scaling; or the server's headless output got "stuck" large from rapid reconnects | `omarchy-dell` uses `/w`/`/h` at the monitor's physical px + `/dynamic-resolution` — the server resizes per connect and fills. Verified on the ultrawide and laptop screen. If it ever looks wrong (e.g. after many quick reconnects), restart the server on dell: `pkill hypr-rdp` then it autostarts, or run `omarchy-rdp-server &`. Don't add `/f` (96×96 mis-probe), `SDL_VIDEO_WAYLAND_SCALE_TO_DISPLAY` (overshoots → cutoff), or `/smart-sizing` (conflicts with `/dynamic-resolution`). |
