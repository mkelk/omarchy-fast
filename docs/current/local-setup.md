...
| **Keystrokes go to local Omarchy, not the remote session** | The XWayland client (`xfreerdp3`) can't receive Wayland keyboard focus under Hyprland, so keys leaked to the compositor. | Fixed: `omarchy-dell` uses the **native Wayland client** (`sdl-freerdp3`), which Hyprland gives real keyboard focus — typing reaches the remote. Your **local SUPER shortcuts stay local** (SUPER+F to un-fullscreen, switch workspace, SUPER+W to close), so you're never trapped and there's nothing to "release". |
